sap.ui.define([
	"justicia/controller/BaseController",
	...
	return BaseController.extend("justicia.controller.OrderDetails", {
		...
		_onObjectMatched: function (oEvent) {
			...
			let oObjectContext = this.getView().getModel("mainService").getContext("/Orders("+oArg.OrderId+")").getObject();
		...
		factoryOrderDetails: function (listId, oContext) {
			...
			//oEvent.getSource().getBindingContext("mainService").getObject()
			let oObjectContext = oContext.getObject();
				oObjectContext.Currency = "EUR";

			let iUnitsInStock = oContext.getModel().getProperty("/Pernr("+oObjectContext.ProductID+")/UnitsInStock");

			if (oObjectContext.Quantity <= iUnitsInStock) {
				let oObjectListItem = new sap.m.ObjectListItem({
					title: "{mainService>/Pernr("+oObjectContext.ProductID+")/ProductName} ({mainService>Quantity})",
					number: "{parts:[{path:'mainService>UnitPrice'},{path:'mainService>Currency'}],type:'sap.ui.model.type.Currency',formatOptions:{showMeasure:false}}",
					numberUnit:"{mainService>Pernr}"
				});
				...
				let oCustomListItem = new sap.m.CustomListItem({
						content:[
							new sap.m.Bar({
								contentLeft: new sap.m.Label({text:"{mainService>/to_Pa0910("+oObjectContext.ProductID+")/PERNR} ({mainService>Pernr})"}),
								contentMiddle: new sap.m.ObjectStatus({text:"{i18n>availableStock} {mainService>/Products("+oObjectContext.ProductID+")/UnitsInStock}", state:"Error"}),
								contentRight: new sap.m.Label({text:"{parts:[{path:'mainService>Pernr'},{path:'mainService>Currency'}],type:'sap.ui.model.type.Currency'}"})
							})
						]
				});	
		...
		onSaveSignature: function (oEvent) {
			...
			if (!oSignature.isFill()) {
				...
				let oBindingContext = oEvent.getSource().getBindingContext("mainService");
				let oData = {
					OrderId: oBindingContext.getProperty("Pernr").toString(),
					SapId:	this.getOwnerComponent().SapId,
					EmployeeId: oBindingContext.getProperty("Pernr").toString(),
					MimeType: "image/png",
		...
		onFileBeforeUpload: function (oEvent) {
			...
			let oItem = oEvent.getParameter("item"),
				...
				oBindingContext = oItem.getBindingContext("mainService"),
				sOrderId = oBindingContext.getProperty("Pernr").toString(),
				sSapId = this.getOwnerComponent().SapId,
				sEmployeId = oBindingContext.getProperty("Pernr").toString(),
